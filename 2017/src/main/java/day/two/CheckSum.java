...
public class CheckSum {
	...
	public static void main(final String[] args) throws UnsupportedEncodingException, IOException {
		...
		final int sum = Arrays.stream(inputs)
				.map((input) -> Arrays.stream(input.split("\\s+")).mapToInt(Integer::parseInt)
						.collect(MaxDiff::new, MaxDiff::accept, MaxDiff::combine).diff())
	...
	static class MaxDiff implements IntConsumer {
		...
		public void combine(final MaxDiff other) {
