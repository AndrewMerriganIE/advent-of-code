...
public class EvenCheckSum {
	...
	public static void main(final String[] args) throws UnsupportedEncodingException, IOException {
		...
		for (final String input : inputs) {
			final List<Integer> sortedInput = Arrays.asList(input.split("\\s+")).stream().map(Integer::parseInt)
					.sorted().collect(Collectors.toList());
			
			outer: for (int smallIndex = 0; smallIndex < sortedInput.size(); smallIndex++) {
				final int smallValue = sortedInput.get(smallIndex);

				for (int largeIndex = 0; largeIndex < sortedInput.size(); largeIndex++) {
					if(largeIndex == smallIndex){
						continue;
					}
					
					final int largeValue = sortedInput.get(largeIndex);

					if (largeValue % smallValue == 0) {
						System.out.println("large:: "+largeValue);
						System.out.println("small:: "+smallValue);
						System.out.println("small:: "+largeValue / smallValue);
						sum += largeValue / smallValue;
						break outer;
					}
				}
			}
	...
	static String[] inputs = { "1208	412	743	57	1097	53	71	1029	719	133	258	69	1104	373	367	365",
